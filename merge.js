...
class address {
    ...
    getWordsDict() {
        ...
        for (var i in elementsList) {
            ...
            // Стоп-элементы
            var rePlace = /^(область|обл\.?|город|г\.?|г|гор\.?|село|сел\.?|сел|селение|поселение|поселок|посёлок|пос\.?|пос|улица|ул\.?|ул|переулок|пер.?|пер-к|пл|пл\.?|п|п\.?|проезд|пр\.?|пр|пр-д|дом|д\.?|д|строение|стр\.?|стр|с\.?|c|корпус|корп\.?|корп|к)$/i;
            // Стоп-предлоги
    ...
    getNumsDict() {
        var elements = {};
        var matches = this.name.match(/[0-9]+/g);
